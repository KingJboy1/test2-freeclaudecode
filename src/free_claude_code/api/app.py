...
class AdminSecurityHeadersMiddleware:
    """Pure ASGI middleware that adds security headers to /admin responses."""
            ...
            if message["type"] == "http.response.start":
                ...
                for key, value in security:
                    if key not in existing:
                        message.setdefault("headers", []).append((key, value))
            await send(message)
...
def create_app(services: ApiServices) -> FastAPI:
    ...
    # BUG #15 fix: add security headers to admin responses (pure ASGI middleware).
