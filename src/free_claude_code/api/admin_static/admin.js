...
function renderProviders() {
  ...
  grid.innerHTML = providers.map(
    (p) => {
      ...
      return `<article class="provider-card" data-provider="${p.id}" data-status="${sc}" data-name="${esc(p.name || p.id)}">
        <div class="provider-title">
          <strong>${esc(p.name || p.id)}</strong>
          <span class="status-pill ${sc}">${statusLabel(p.status)}</span>
        </div>
        <div class="provider-meta">${esc(p.description || p.status_detail || "")}</div>
        <button class="test-button" onclick="testProvider('${esc(p.id)}')">Test</button>
      </article>`;
    }
  ).join("");
  updateProviderCount();
