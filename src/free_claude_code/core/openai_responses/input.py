...
# Maximum base64 image payload: 20 MB decoded.
...
def _convert_input_image_part(part: Mapping[str, Any]) -> dict[str, Any]:
    """Convert an OpenAI Responses ``input_image`` part to an Anthropic image block.

    Supports both ``image_url`` (data-URI or HTTP URL) and inline ``image``
    (base64) fields.  Falls back to a text placeholder for invalid payloads
    so the request is never silently dropped.
    """
    image_url = part.get("image_url") or part.get("url") or ""
    if isinstance(image_url, str) and image_url:
        match = _DATA_URI_RE.match(image_url)
        if match:
            ...
            try:
                ...
                return {
                    "type": "text",
                    "text": "[invalid image: could not decode base64]",
                }
            if len(raw) > _MAX_IMAGE_BYTES:
                return {"type": "text", "text": "[invalid image: exceeds 20 MB limit]"}
            return {
                "type": "image",
    ...
    # Inline base64 via "image" field (some clients use this).
    inline_b64 = part.get("image") or part.get("data") or ""
    if isinstance(inline_b64, str) and inline_b64:
        mime = part.get("media_type", "image/png")
        try:
            raw = base64.b64decode(inline_b64, validate=True)
        except Exception:
            return {"type": "text", "text": "[invalid image: could not decode base64]"}
        if len(raw) > _MAX_IMAGE_BYTES:
            return {"type": "text", "text": "[invalid image: exceeds 20 MB limit]"}
        return {
            "type": "image",
            "source": {"type": "base64", "media_type": mime, "data": inline_b64},
        }

    return {"type": "text", "text": "[invalid image: no recognisable image payload]"}
    ...
    if isinstance(content, list):
        ...
        for part in content:
            ...
            if part_type == "input_image":
                blocks.append(_convert_input_image_part(part))
                continue
            raise ResponsesConversionError(
